...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Save to video map when URL changes
    if (mainVideoUrl) {
  ...
  async function startCamera() {
    try {
      ...
      // Use a small delay to ensure React has mounted the video element
      setTimeout(() => {
        if (cameraVideoRef.current) {
          console.log("📷 Camera: Setting srcObject on video element");
          cameraVideoRef.current.srcObject = stream;
          console.log("📷 Camera: Video element srcObject set:", cameraVideoRef.current.srcObject);
          console.log("📷 Camera: Video element readyState:", cameraVideoRef.current.readyState);
          console.log("✅ Camera: Stream assigned, waiting for play...");
        } else {
    ...
    <div style={styles.container}>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Scrollbar styling - only on main content */
        ::-webkit-scrollbar {
          width: 8px;
          height: 8px;
        }
        ...
        /* Hide scrollbars on component divs that should use page scroll */
        .no-scroll {
      ...
      <div style={styles.mainContent}>
        ...
        <div style={styles.topRow}>
          ...
          <div style={styles.cameraSection}>
            ...
            {cameraActive ? (
              <video ref={cameraVideoRef} muted autoPlay playsInline style={styles.cameraPreview} />
            ) : (
      ...
      {showAddStepDialog && (
        <div style={styles.modalOverlay} onClick={() => setShowAddStepDialog(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.formGroup}>
              <label>Main Video Start Time (mm:ss or seconds)</label>
              <input type="text" placeholder="e.g., 0:15 or 15" value={dialogForm.mainStart} onChange={(e) => setDialogForm({ ...dialogForm, mainStart: e.target.value })} style={{ width: "100%" }} />
              <small style={{ color: "#999" }}>When this step begins in the choreography video</small>
            </div>
            <div style={styles.formGroup}>
              <label>Main Video End Time (mm:ss or seconds)</label>
              <input type="text" placeholder="e.g., 0:30 or 30" value={dialogForm.mainEnd} onChange={(e) => setDialogForm({ ...dialogForm, mainEnd: e.target.value })} style={{ width: "100%" }} />
              <small style={{ color: "#999" }}>When this step ends in the choreography video</small>
            </div>
            ...
            {dialogForm.tutorialUrl && (
              <>
                <div style={styles.formGroup}>
                  <label>Tutorial Start Time (mm:ss or seconds)</label>
                  <input type="text" placeholder="e.g., 0:05 or 5" value={dialogForm.tutorialStart} onChange={(e) => setDialogForm({ ...dialogForm, tutorialStart: e.target.value })} style={{ width: "100%" }} />
                  <small style={{ color: "#999" }}>Where in tutorial to start</small>
                </div>
                <div style={styles.formGroup}>
                  <label>Tutorial End Time (mm:ss or seconds)</label>
                  <input type="text" placeholder="e.g., 0:20 or 20" value={dialogForm.tutorialEnd} onChange={(e) => setDialogForm({ ...dialogForm, tutorialEnd: e.target.value })} style={{ width: "100%" }} />
                  <small style={{ color: "#999" }}>Where in tutorial to end</small>
                </div>
              </>
            )}
            <div style={styles.divider} />
...
const styles = {
  ...
  headerSelect: { padding: "8px 12px", border: "2px solid #B0B8C8", borderRadius: "6px", fontSize: "13px", backgroundColor: "#fff", color: "#2A1F1F", cursor: "pointer" },
  ...
  stepItem: { display: "grid", gridTemplateColumns: "1fr auto", gap: "8px", padding: "12px", backgroundColor: "#f9f9f9", border: "2px solid #B0B8C8", borderRadius: "6px", alignItems: "center" },
  ...
  btn: { padding: "10px 16px", borderRadius: "6px", fontSize: "13px", fontWeight: "600", cursor: "pointer", border: "2px solid transparent" },
  btnPrimary: { backgroundColor: "#FF8A5B", color: "#fff" },
  btnSecondary: { backgroundColor: "#FFE0CC", color: "#2A1F1F", border: "1px solid #FFB8A0" },
  btnSuccess: { backgroundColor: "#FF8A5B", color: "#fff" },
  ...
  divider: { height: "1px", backgroundColor: "#E0DDD5", margin: "12px 0" },
